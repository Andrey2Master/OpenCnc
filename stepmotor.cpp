...
// TIM5 - motor 3
 ...
 TIM1->EGR = TIM_PSCReloadMode_Update; TIM_ARRPreloadConfig(TIM1, ENABLE);
 TIM_ITConfig(TIM1, TIM_IT_Update, ENABLE); TIM_Cmd(TIM1, ENABLE);
  ...
  step_motors[id].clk = !step_motors[id].clk;
		...
		if(steps_buf_sz > 0 && !step_motors[0].isInProc && !step_motors[1].isInProc &&
	 	...
	 	if(step_motors[3].steps) {
	 	 ...
	 	 TIM1->PSC = p->pscValue[3];	TIM_SetAutoreload(TIM1, p->arrValue[3]);
	 	}
			steps_buf_p1++;
