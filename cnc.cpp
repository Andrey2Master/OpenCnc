...
double  dx, dy;
uint16_t Pos=0,i;
while(CNC_DATA[Pos].X!=-1)	
// for(i = 0; i < 7; i++)
	{
	dx=(CNC_DATA[Pos].X-Last_X)/80.0;
	dy=(CNC_DATA[Pos].Y-Last_Y)/80.0;
		
 	moveLength = sqrt(float(dx * dx + dy * dy ));
//	moveLength = moveLength/80.0;	
  
	cnc_line(CNC_DATA[Pos].X,CNC_DATA[Pos].Y,CNC_DATA[Pos].X, CNC_DATA[Pos].Y, moveLength,300);  // mm * 60000/(mm/min) = msec	
                 ...
                 double moveLength, double feed_rate) 
{
 ...
 int32_t newZ;// = lround(z*SM_Z_STEPS_PER_MM);
 int32_t newE;//= lround(extruder_length*SM_E_STEPS_PER_MM);
...
// if((curGCodeMode & GFILE_MODE_MASK_EXEC) != 0) {
 	return smothLine(dx, dy, dx, dy, time_msec, moveLength, (uint32_t)feed_rate);
// }
// return TRUE;
}
/*******************************************************************************************************/
